...
class ProductService
{
    ...
    public function getProducts(int $partner_id, Request $request): JsonResponse
    {
        ...
        $category_ids = $request->category_ids ? (!is_array($request->category_ids) ? json_decode($request->category_ids,1) : $request->category_ids) : [];
        $collection_ids = $request->collection_ids ? (!is_array($request->collection_ids) ? json_decode($request->collection_ids,1) : $request->collection_ids) : [];
        $price_range = $request->price_range ? (!is_array($request->price_range) ? json_decode($request->price_range,1) : $request->price_range) : [];
        $ratings = $request->ratings ? (!is_array($request->ratings) ? json_decode($request->ratings,1) : $request->ratings) : [];
        ...
        $this->productList->setPartnerId($partner_id)
            ...
            ->setRatings($ratings)
            ->setSearchKey($request->search_key)
            ->setOffset($offset)
            ->setLimit($limit);
